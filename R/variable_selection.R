model_select <- function(framework, sigmau2, method, interval,
  ...
  if (framework$correlation == "spatial"){
    Vi  <- solve(eblup$V)
    loglike<- (-0.5) * ( m * log(2*pi) + determinant(eblup$V,logarithm=TRUE)$modulus +
                           t(res)%*%Vi%*%res)
    AIC    <-(-2) * loglike + 2 * (p + 2)
  ...
  if ((framework$correlation == "no" & transformation == "no") & !is.null(B)){
  ...
  for (b in seq_len(B)){
   ...
   direct.boot <- framework$model_X%*%eblup$coefficients$coefficients + u.boot + e.boot
   ...
   eblup.boot <- eblup_FH(framework = framework.boot, sigmau2 = sigmau2.boot,
                     combined_data = combined_data)
   
   B1[b] <- sum(((sigmau2 + framework$vardir) + 
               ((framework$model_X%*%(eblup$coefficients$coefficients - 
                                 eblup.boot$coefficients$coefficients))^2) - 
               ((direct.boot - framework$model_X%*%eblup.boot$coefficients$coefficients)^2))/
                 (sigmau2.boot + framework$vardir))
   
   loglike.boot[b] <- -(framework$m/2*log(2*pi)) - 
                    (1/2*sum(log(sigmau2.boot + framework$vardir))) -
                    (1/2*sum((framework$direct  - 
                                framework$model_X%*%eblup.boot$coefficients$coefficients)^2/
                               (sigmau2.boot + framework$vardir)))
   
   res.boot <- eblup.boot$real_res + eblup.boot$random_effects
   loglikestar.boot[b] <- (-0.5) * (sum(log(2 * pi * (sigmau2.boot + framework$vardir)) +
  ...
  AICb1 <- (-2)*loglike - 2*(mean(loglike.boot) - mean(loglikestar.boot))
